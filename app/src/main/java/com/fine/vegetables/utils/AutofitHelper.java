...
public class AutofitHelper {
    ...
    public static AutofitHelper create(TextView view, AttributeSet attrs, int defStyle) {
        ...
        if (attrs != null) {
            ...
            int maxViewWidth = helper.getMaxViewWidth();
            ...
            maxViewWidth = ta.getDimensionPixelSize(R.styleable.AutofitTextView_maxViewWidth,
                    maxViewWidth);
            ta.recycle();

            helper.setMinTextSize(TypedValue.COMPLEX_UNIT_PX, minTextSize)
                    .setPrecision(precision)
                    .setMaxViewWidth(maxViewWidth);
        }
    ...
    private static void autofit(TextView view, TextPaint paint, float minTextSize, float maxTextSize,
                                int maxLines, float precision, int maxViewWidth) {
        ...
        if ((maxLines == 1 && paint.measureText(text, 0, text.length()) > targetWidth)
            ...
            Log.d(TAG, "autofit: " + text + "，measure：" + paint.measureText(text, 0, text.length()) + "，targetWidth：" + targetWidth + "，maxWidth: " + maxViewWidth);
            Log.d(TAG, "autofit: getLineCount" + getLineCount(text, paint, size, targetWidth, displayMetrics));
    ...
    public AutofitHelper setMaxViewWidth(int maxViewWidth) {
        if (mMaxViewWidth != maxViewWidth) {
    ...
    private void autofit() {
        ...
        mIsAutofitting = true;
        autofit(mTextView, mPaint, mMinTextSize, mMaxTextSize, mMaxLines, mPrecision, mMaxViewWidth);
